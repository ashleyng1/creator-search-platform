...
export function CreatorAvatar({ handle, name, size = 36 }: { handle: string; name?: string; size?: number }) {
  return (
    <Image
      src={creatorAvatarUrl(handle, name)}
      alt={handle}
      width={size}
      height={size}
      className="rounded-full bg-brand-100"
      unoptimized
    />
  );
}
...
export function CreatorListRow({ creator, selected, onToggle, index }: Props) {
  ...
  return (
    <tr className={`hover:bg-surface-muted/60 ${selected ? "bg-brand-50/50" : ""}`}>
      <td className="w-10 text-center text-xs text-gray-400">{index}</td>
      <td className="w-10">
        <input
          type="checkbox"
          checked={selected}
      ...
      <td>
        <div className="flex items-center gap-3">
          <CreatorAvatar handle={creator.handle} name={creator.display_name} />
          <div>
            <a
      ...
      <td className="text-gray-600">{creator.audience_country || "—"}</td>
      <td>
        <span className="tag-purple">{tier}</span>
      </td>
      <td className="text-gray-600">{creator.categories.split("|")[0] || creator.categories}</td>
  ...
  return (
    ...
    >
      <input
        ...
        checked={selected}
        onChange={onToggle}
        className="absolute left-3 top-3 h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500"
      />
      <div className="flex flex-col items-center pt-2 text-center">
        <CreatorAvatar handle={creator.handle} name={creator.display_name} size={64} />
        <h3 className="mt-3 font-medium text-gray-900">{creator.display_name}</h3>
        <p className="text-xs text-gray-500">@{creator.handle}</p>
        ...
        <p className="mt-2 line-clamp-2 text-xs text-gray-500">{creator.categories}</p>
        <div className="mt-3 grid w-full grid-cols-3 gap-2 border-t border-surface-border pt-3 text-center text-xs">
          <div>
            <p className="font-semibold text-gray-900">{formatFollowers(creator.followers)}</p>
            <p className="text-gray-400">Followers</p>
          </div>
          <div>
            <p className="font-semibold text-gray-900">{(creator.engagement_rate * 100).toFixed(1)}%</p>
            <p className="text-gray-400">ER</p>
          </div>
          <div>
            <p className="font-semibold text-gray-900 truncate">{creator.audience_country || "—"}</p>
